          ...
          if classes[i] in category_index.keys():
            ...
            if(class_name == "person"):
              ...
              if(count[i] == 0):
                ...
                name[i] = ''.join([random.choice(string.ascii_letters + string.digits) for n in range(8)])
                
                try:
                  if not os.path.exists("StoreImage/"+name[i]):
                    os.makedirs("StoreImage/"+name[i])
                except OSError:
                  print ('Error: Creating directory. ' + directory)
                count[i] = count[i] + 1
                cv2.imwrite("StoreImage/"+name[i]+"/"+str(count[i])+".png",image[int(ymin*im_height):int((ymax)*im_height),int(xmin*im_width):int((xmax)*im_width)])
                print(name[i])
              else :
                count[i] = count[i] + 1
                cv2.imwrite("StoreImage/"+name[i]+"/"+str(count[i])+".png",image[int(ymin*im_height):int((ymax)*im_height),int(xmin*im_width):int((xmax)*im_width)])
                Img1 = cv2.imread('StoreImage/'+name[i]+'/'+str(count[i]-1)+'.png',3)
                Img2 = cv2.imread('StoreImage/'+name[i]+'/'+str(count[i])+'.png',3)
                Img1 = cv2.cvtColor(Img1,cv2.COLOR_BGR2GRAY)
                Img2 = cv2.cvtColor(Img2,cv2.COLOR_BGR2GRAY)
                Img1KP = sift.detect(Img1,None)
                Img1KP,Img1Desc = sift.compute(Img1,Img1KP)


                Img2KP = sift.detect(Img2,None)
                Img2KP,Img2Desc = sift.compute(Img2,Img2KP)

                matches = flann.knnMatch(Img1Desc,Img2Desc,k=2)
                ...
                if(count[i] == 15):
                  vid_path = urlStore+name[i]+'*'
                  url = API_URL+vid_path
                  status = True
